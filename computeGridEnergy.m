...
for i1 = 1:Nt
    obs.gridEnergy(:,:,i1) = interp1(obs.rp,obs.totalEnergy(i1,:),rg);
end
if ~acoustics
    obs.gridEnergyP = zeros(length(xg),length(xg),Nt);
    for i1 = 1:Nt
        obs.gridEnergyP(:,:,i1) = interp1(obs.rp,obs.sensorEnergyP(:,i1),rg);
    end
    obs.gridEnergyS = zeros(length(xg),length(xg),Nt);
    for i1 = 1:Nt
        obs.gridEnergyS(:,:,i1) = interp1(obs.rp,obs.sensorEnergyS(:,i1),rg);
    end
end
end