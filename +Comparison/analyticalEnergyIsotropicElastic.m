function E = analyticalEnergyIsotropicElastic(geometry, material, observation, r, Wsp)
% This function computes (semi-)analytical energy densities of P and S-waves
% in 2D and total energy density in 3D for elastic isotropic scattering media, 
% based on :
% (1) H. Nakahara, K. Yoshimoto. Radiative transfer of elastic waves in
%     two-dimensional isotropic scattering media: semi-analytical approach
%     for isotropic source radiation. Earth Planets Space 63, pp. 459-468 (2011).
% (2) H. Sato. Multiple isotropic scattering model including P-S conversions
%     for the seismogram envelope formation. Geophys. J. Int 117, pp. 487-494 (1994).

% Inputs
% geometry, material, obvervation 
%                    : same as those for the radiative transfer solver
% r                  : source-station distance
% Wsp                : S to P energy ratio at the source position

% Output
% E : total energy density in 3D and those for each polarization type in 2D
%     (first dimension = 'P', second dimension = 'S')

% Assumption : Following the cited papers, this code assumes that:
%              Sigmap = Sigmas = eta
...
% normalized time = time/mean free time (a = eta*t)
a = eta*t;
% normalized distance = (eta/vp)*distance
b = (eta/vp)*r;

% Functions that will be further used
...
if d==2
    ...
    Ep0 = exp(-a)./(2*pi*b').*deltaFunction(a-b',width);
    Es0 = Wsp*K*exp(-a)./(2*pi*b').*deltaFunction(a-K*b',width);

    % Single scattering
    Epp1 = (Sigmapp/eta)*exp(-a)./(2*pi*b'.*sqrt((a./b').^2 - 1)).*H(a-b');
    Esp1 = Wsp*K*(Sigmasp/eta)*exp(-a).*M_func(a./b',K)./(pi^2*b').*H(a-b');

    Ep1 = Epp1 + Esp1;

    Ess1 = (Sigmass/eta)*Wsp*K*exp(-a)./(2*pi*b'.*sqrt((a./K/b').^2 - 1)).*H(a-K*b');
    Eps1 = 1*K*(Sigmaps/eta)*exp(-a).*M_func(a./b',K)./(pi^2*b').*H(a-b');
    ...
    Emp = @(k,s,K) ( 1*Gp(k,s).*( (Sigmapp*Gp(k,s)).^2.*(1-Sigmass*Gs(k,s,K)) + ...
        ...
        + Wsp*Gs(k,s,K).*( Sigmasp*Gp(k,s).*( Sigmapp*Gp(k,s).*...
    ...
    Ems = @(k,s,K) ( Wsp*Gs(k,s,K).*( (Sigmass*Gs(k,s,K)).^2.*(1-Sigmapp*Gp(k,s)) + ...
    ...
    kk = [0 besselroots(0,20)']; Nk = length(kk);
    Nw = 2^12; Lw = 50; ww = linspace(-Lw/2,Lw/2,Nw); dww = Lw/Nw;
    dt = 2*pi/Lw; tt = (-Nw/2:Nw/2-1)*dt;

    out1 = zeros(Nw,Nk-1); 
    fun = @(x,w) x.*besselj(0,x).*Emp(x/b,-1i*w,K);
    parfor j=1:Nk-1
        out1(:,j) = integral(@(x) fun(x,ww'),kk(j),kk(j+1),'RelTol',1e-13,'AbsTol',1e-13,'ArrayValued',true);
    end
    temp = forwardAverage(cumsum(out1,2),Nk-2);
    
    Epm = (1/b'.^2)*(1/(2*pi)^2)*interp1(tt,abs(dww*fftshift(fft(temp(:,end)))),a);
    Epm(~H(a-b')) = 0;
    Ep = Ep0 + Ep1 + Epm;

    out1 = zeros(Nw,Nk-1); 
    fun = @(x,w) x.*besselj(0,x).*Ems(x/b,-1i*w,K);
    parfor j=1:Nk-1
        out1(:,j) = integral(@(x) fun(x,ww'),kk(j),kk(j+1),'RelTol',1e-13,'AbsTol',1e-13,'ArrayValued',true);
    end
    temp = forwardAverage(cumsum(out1,2),Nk-2);

    Esm = (1/b'.^2)*(1/(2*pi)^2)*interp1(tt,abs(dww*fftshift(fft(temp(:,end)))),a);
    Esm(~H(a-b')) = 0;
    Es = Es0 + Es1 + Esm;

    E = zeros(length(t),2);
    E(:,1) = Ep*(eta/vp)^d; E(:,2) = Es*(eta/vp)^d;

elseif d == 3
    % Direct waves (no scattering)
    % Gaussian pulse used to define a Dirac delta function
    E0 = (deltaFunction(a-b',width) + Wsp*deltaFunction(a-K*b',width)) ...
                               .*exp(-a).*1./(4*pi*b'.^2*(1+Wsp));
    % Single scattering
    Epp1 = 1*Sigmapp*K_func(a./b');
    Epp1(~H(a-b')) = 0;

    Eps1 = 1*K*Sigmaps*Kc_func(a./b',K);
    Eps1(a./b'==1) = 1*K*Sigmaps*atanh(1/K);
    Eps1(~H(a-b')) = 0;

    Esp1 = Wsp*K*Sigmasp*Kc_func(a./b',K);
    Esp1(a./b'==1) = Wsp*K*Sigmasp*atanh(1/K);
    Esp1(~H(a-b')) = 0;

    Ess1 = Wsp*K*Sigmass*K_func(a/K./b');
    Ess1(~H(a-K*b')) = 0;

    E1 = (Epp1+Eps1+Esp1+Ess1).*exp(-a)./(4*pi*b'.^2*eta*(1+Wsp));
    ...
    Em = @(k,s,K) (1*Gp(k,s).*(Sigmaps*Gs(k,s,K).*(Sigmass*Gs(k,s,K).*(1-Sigmapp*Gp(k,s))+Sigmasp*Gp(k,s).*(1+Sigmaps*Gs(k,s,K))) + ...
        ...
        +Wsp*Gs(k,s,K).*(Sigmasp*Gp(k,s).*(Sigmapp*Gp(k,s).*(1-Sigmass*Gs(k,s,K))+Sigmaps*Gs(k,s,K).*(1+Sigmasp*Gp(k,s))) + ...
    ...
    Em0 = @(s,K) (1*(1/eta./(s+1)).*(Sigmaps*(1/eta./(s+1)).*(Sigmass*(1/eta./(s+1)).*(1-Sigmapp*(1/eta./(s+1)))+Sigmasp*(1/eta./(s+1)).*(1+Sigmaps*(1/eta./(s+1)))) + ...
        ...
        +Wsp*(1/eta./(s+1)).*(Sigmasp*(1/eta./(s+1)).*(Sigmapp*(1/eta./(s+1)).*(1-Sigmass*(1/eta./(s+1)))+Sigmaps*(1/eta./(s+1)).*(1+Sigmasp*(1/eta./(s+1)))) + ...
    ...
    [Xq,Yq] = meshgrid(b,a);
    E_interp = interp2(rr,tt,E_fourier,Yq,Xq);

    E_multiple = eta./((1+Wsp)*(2*pi)^2*b').*E_interp';
    E_multiple(~H(a-b')) = 0;

    E = (E0 + E1 + E_multiple)*(eta/vp)^d;
